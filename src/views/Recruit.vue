  ...
  <div class="recruit-container">
    ...
    <header class="recruit-header">
      <div class="header-left">
        <button class="header-back-btn" @click="handleExit" title="返回">
      ...
      <div class="header-right">
        ...
        <div class="currency-item ap-pill">
          <div class="currency-icon-wrapper">
            <svg class="currency-svg" viewBox="0 0 24 24" width="16" height="16">
              ...
              <path d="M2 17l10 5 10-5M2 12l10 5 10-5" stroke="#00E676" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none" />
            </svg>
          </div>
        ...
        <div class="currency-item ticket10-pill" >
          <div class="currency-icon-wrapper">
            <img src="/image/recruit_ticket_10.webp" class="currency-icon-img" alt="10-Draw Ticket" />
          </div>
          <span class="currency-val" style="font-weight: 900; font-family: monospace;">INF</span>
        ...
        <div class="currency-item ticket-pill" >
          <div class="currency-icon-wrapper">
            <img src="/image/recruit_ticket_1.webp" class="currency-icon-img" alt="Selection Ticket" />
          </div>
          <span class="currency-val" style="font-weight: 900; font-family: monospace;">INF</span>
        ...
        <!-- Settings Button -->
        <button class="header-setting-btn" @click="handleOpenSettings" title="设置">
        ...
        <!-- Home Button -->
        <button class="header-home-btn" @click="handleExit" title="主页">
          <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <!-- Main Screen (Left Sidebar & Right Stage) -->
      ...
      <aside class="recruit-sidebar">
        <div class="sidebar-tabs-container">
          <div
            ...
            :class="[{ 'is-active': activePoolIndex === idx }]"
            @click="switchPool(idx)"
          >
            <div class="tab-banner-image" :style="pool.tabAvatar ? `background-image: url('${pool.tabAvatar}');` : ''">
              <div v-if="!pool.tabAvatar" class="tab-banner-fallback" :class="`fallback-${pool.tabType}`">
                <span class="fallback-main">{{ pool.name }}</span>
                <span class="fallback-sub">{{ pool.tabName }}</span>
              </div>
            </div>
            <div class="tab-active-glow"></div>
          </div>
        </div>
      ...
      <main class="recruit-stage">
        ...
        <div class="stage-bg-container">
          ...
          <div v-else-if="currentPool?.bgImage" class="stage-bg-image" :style="`background-image: url('${currentPool.bgImage}');`"></div>
          <div v-else class="ba-css-bg">
            <div class="ba-bg-grids"></div>
            <div class="ba-bg-stripes"></div>
            <div class="ba-bg-rays"></div>
            <div class="ba-bg-shards">
              <div class="ba-shard ba-shard-1"></div>
              <div class="ba-shard ba-shard-2"></div>
              <div class="ba-shard ba-shard-3"></div>
              <div class="ba-shard ba-shard-4"></div>
            </div>
          </div>
        </div>

        <!-- Info Overlay with white bottom gradient -->
        <div class="stage-overlay">
          <div class="pool-text-info">
            <h2 class="pool-title">{{ currentPool?.name || '★3学生自选招募' }}</h2>
            <div class="pool-period">
              <span>{{ currentPool?.startTime || '2025/07/22 11:00' }} ～ {{ currentPool?.endTime || '2026/06/24 10:59' }}截止</span>
            </div>
          </div>
          ...
          <div class="stage-actions-row">
            <!-- Magnifying Glass -->
            <button class="rates-search-btn" @click="showRatesModal" title="成员一览">
              <div class="rates-btn-box">
                <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="3">
                  <circle cx="10" cy="10" r="6"/><line x1="21" y1="21" x2="15" y2="15"/>
                </svg>
              </div>
            </button>
            ...
            <div class="gacha-buttons-wrapper">
              <template v-if="currentPool?.gachaType === 'gacha'">
                <button class="gacha-btn btn-single" @click="handleGacha(1)">
                  <div class="gacha-btn-inner gacha-classic">
                    <div class="gacha-left-icon">
                      <img src="/image/recruit_envelope_1.png" alt="1 Draw" />
                    </div>
                    <div class="gacha-right-content">
                      <div class="gacha-cost-pill">
                        <img :src="true ? '/image/recruit_ticket_1.webp' : '/image/pyroxene.png'" class="btn-cost-icon" alt="cost" />
                        <span>{{ true ? '1' : '120' }}</span>
                      </div>
                      <span class="gacha-btn-title">{{ currentPool.buttonText1 || '招募1次' }}</span>
                    </div>
                  </div>
                </button>
                <button class="gacha-btn btn-multi" @click="handleGacha(10)">
                  <div class="gacha-btn-inner gacha-classic">
                    <div class="gacha-left-icon">
                      <img src="/image/recruit_envelope_10.png" alt="10 Draw" />
                    </div>
                    <div class="gacha-right-content">
                      <div class="gacha-cost-pill">
                        <img :src="true ? '/image/recruit_ticket_10.webp' : '/image/pyroxene.png'" class="btn-cost-icon" alt="cost" />
                        <span>{{ true ? '1' : '1,200' }}</span>
                      </div>
                      <span class="gacha-btn-title">{{ currentPool.buttonText2 || '招募10次' }}</span>
                    </div>
                  </div>
                </button>
              </template>
              <template v-else-if="currentPool?.gachaType === 'select'">
                <!-- Buy Ticket (Yellow / btn-multi style) -->
                <div class="gacha-btn btn-multi" style="overflow: visible;" @click="openReplenish('ticket')">
                  <div class="btn-ticket-bubble">距离结束剩余<span class="tag-highlight">32天</span></div>
                  <div class="gacha-btn-inner gacha-classic">
                    <div class="gacha-left-icon" style="width: 110px; margin-top: -10px; margin-left: -15px;">
                      <img src="/image/select_ticket.png" alt="Buy Ticket" style="transform: rotate(-5deg); filter: drop-shadow(0 4px 6px rgba(0,0,0,0.3));" />
                    </div>
                    <div class="gacha-right-content">
                      <div class="gacha-cost-pill">
                        <img src="/image/credit.png" class="btn-cost-icon" alt="cost" />
                        <span>3,000</span>
                      </div>
                      <span class="gacha-btn-title">{{ currentPool.buttonText1 || '购买自选券' }}</span>
                    </div>
                  </div>
                </div>
                ...
                <div class="gacha-btn btn-single" @click="openSelectionModal">
                  <div class="gacha-btn-inner gacha-classic">
                    <div class="gacha-left-icon" style="width: 55px; margin-left: 15px; margin-top: 0;">
                      <img src="/image/select_ticket.png" alt="Use Ticket" style="transform: rotate(-10deg);" />
                    </div>
                    <div class="gacha-right-content">
                      <div class="gacha-cost-pill">
                        <span>1</span>
                      </div>
                      <span class="gacha-btn-title">{{ currentPool.buttonText2 || '使用自选券' }}</span>
                    </div>
                  </div>
                </div>
              </template>
    ...
    <!-- Replenish Dialog -->
    <div class="ba-modal-overlay" v-if="showReplenishDialog">
      <div class="ba-dialog-box animate-pop">
        <div class="ba-dialog-header">阿罗娜的补给箱～</div>
        <div class="ba-dialog-content">
          <p v-if="replenishTarget === 'pyroxene'">老师，要阿罗娜为您补充 <span class="highlight">1,200 青辉石</span> 吗？</p>
          <p v-else-if="replenishTarget === 'credit'">老师，要阿罗娜为您充值 <span class="highlight">10,000,000 信用积分</span> 吗？</p>
          <p v-else-if="replenishTarget === 'ap'">老师，要阿罗娜为您补充 <span class="highlight">120 AP</span> 吗？</p>
          <p v-else-if="replenishTarget === 'recruitTicket1'">老师，要阿罗娜为您补充 <span class="highlight">10张 1次招募券</span> 吗？</p>
          <p v-else-if="replenishTarget === 'recruitTicket10'">老师，要阿罗娜为您补充 <span class="highlight">2张 10次招募券</span> 吗？</p>
          <p v-else-if="replenishTarget === 'ticket'">老师，要阿罗娜为您补充 <span class="highlight">1张 ★3学生自选券</span> 吗？<br/><small style="color: #8c9ba5;">(需要消耗 30,000,000 信用积分)</small></p>
        </div>
    ...
    <!-- Student Details Modal (Rates list) -->
    <div class="ba-modal-overlay" v-if="showDetailsModal">
      <div class="ba-list-modal animate-pop">
        ...
        <div class="ba-modal-body">
          <div class="ba-rates-summary">
            <span>总人数: {{ students.length }} 人</span>
            <span>由于是完全随机点名，所以当前每个学生的招募权重决定其招募概率哦！</span>
          </div>
          <table class="ba-rates-table">
            <thead>
              <tr>
                <th>学生名字</th>
                <th>学院</th>
    ...
    <!-- Student Selection Ticket Modal -->
    <div class="ba-modal-overlay" v-if="showSelectionModal">
      <div class="ba-list-modal select-modal animate-pop">
        ...
        <div class="ba-modal-body">
          <p class="select-desc">老师可以任意挑选一名已在名单中的学生，使用自选券将直接把她接回学校哦！</p>
          <div class="student-select-grid">
            <div
              v-for="student in students"
              :key="student.name"
              class="student-select-card"
              :class="{ 'is-selected': selectedStudent?.name === student.name }"
            ...
            >
              ...
              <div class="student-info">
                ...
                <span class="student-select-school">{{ student.academy || '未登记学院' }}</span>
              </div>
              <div class="student-select-tick" v-if="selectedStudent?.name === student.name">✓</div>
            </div>
          </div>
        </div>
        <div class="ba-modal-footer">
          <button class="ba-btn-cancel" @click="showSelectionModal = false">先返回</button>
          <button class="ba-btn-confirm" :disabled="!selectedStudent" @click="confirmStudentSelection">确认招募！</button>
        </div>
      </div>
    </div>
  </div>
...
const handleExit = async () => {
  ...
  try {
    ...
    console.error('Failed to close recruit window:', e)
  }
}
...
<style scoped>
/* ==========================================================================
   Blue Archive Recruitment Interface Styling
   ========================================================================== */
...
.recruit-container {
  ...
  --ba-muted: #8CA3BF;
  --ba-white: #FFFFFF;
  --ba-trans-dark: rgba(26, 58, 92, 0.85);
...
.currency-add-btn {
  ...
  border: 1px solid var(--ba-gold-dark);
  color: var(--ba-dark);
  font-weight: 900;
  font-size: 15px;
  display: flex;
...
.recruit-workspace {
  ...
  background: linear-gradient(135deg, #B5CCEB 0%, #D8E4F8 100%);
...
.sidebar-tab-item:not(.is-active):hover {
...
.sidebar-tab-item.is-active {
  border-color: #FFDE00;
  box-shadow: 0 0 12px rgba(255, 222, 0, 0.8), 0 4px 10px rgba(0,0,0,0.15);
...
.tab-banner-fallback {
  ...
  align-items: center;
  justify-content: center;
  background: linear-gradient(135deg, #F0F6FA 0%, #DCE7F3 100%);
}
...
.fallback-pickup_blue { background: linear-gradient(135deg, #E6F3FF 0%, #B3D9FF 100%); }
.fallback-pickup_pink { background: linear-gradient(135deg, #FFE6EE 0%, #FFB3CC 100%); }
.fallback-select { background: linear-gradient(135deg, #FFF9E6 0%, #FFE680 100%); }

.selectable-members-btn {
...
.stage-overlay {
  ...
  padding: 30px;
  /* Strong white to transparent gradient from bottom */
  background: linear-gradient(to top, rgba(255,255,255,1) 0%, rgba(255,255,255,0.85) 15%, rgba(255,255,255,0) 40%);
}
...
.pool-text-info {
  ...
  margin-bottom: 15px; /* Slight margin to align baseline beautifully with buttons */
}
...
.rates-search-btn:hover .rates-btn-box {
  background: #F0F6FC;
  transform: translateY(-2px);
}

/* Gacha button styling */
.gacha-buttons-wrapper {
  display: flex;
  gap: 12px;
}
...
.btn-buy-ticket-bottom {
  height: 50%;
  background: #FFD400;
  display: flex;
  align-items: center;
  justify-content: center;
  color: #0E365C; /* Dark blue text */
  transform: skewX(15deg);
  width: 120%;
  margin-left: -10%;
}
...
.btn-use-ticket {
  background: linear-gradient(135deg, #FFF96E 0%, #FFCC22 100%);
  border: none;
  color: #4A1A04; /* Dark brown text */
}

.btn-use-ticket:hover {
  transform: skewX(-15deg) translateY(-2px);
  box-shadow: 0 8px 18px rgba(255, 204, 34, 0.4);
}

.btn-buy-ticket:hover {
  transform: skewX(-15deg) translateY(-2px);
  box-shadow: 0 8px 18px rgba(255, 212, 0, 0.4);
}
...
.btn-single {
  background: linear-gradient(135deg, #7BE4FF 0%, #46C4FF 100%);
  border: none;
  width: 220px;
}
.btn-single:hover {
  transform: skewX(-15deg) translateY(-2px);
  box-shadow: 0 6px 14px rgba(70, 196, 255, 0.4);
}
.btn-single .gacha-cost-pill {
  background: #113854;
  color: #FFFFFF;
}
.btn-single .gacha-btn-title {
  color: #0E365C;
}
...
.btn-multi:hover {
  transform: skewX(-15deg) translateY(-2px);
  box-shadow: 0 6px 14px rgba(255, 204, 34, 0.4);
}
.btn-multi .gacha-cost-pill {
  background: #3B1C10;
  color: #FFFFFF;
}
.btn-multi .gacha-btn-title {
  color: #4A1A04;
}

.gacha-btn-inner.gacha-classic {
  flex-direction: row;
  justify-content: space-between;
  padding: 0 10px 0 0;
}
...
.btn-ticket-bubble {
  ...
  font-weight: 800;
  z-index: 10;
  transform: skewX(15deg); /* Un-skew from parent */
  white-space: nowrap;
  box-shadow: 0 4px 8px rgba(0,0,0,0.3);
}
.btn-ticket-bubble::after {
  content: '';
  position: absolute;
  bottom: -4px;
  left: 18px;
...
/* Modals overlays & dialogs styling */
.ba-modal-overlay {
  position: absolute;
  inset: 0;
...
.ba-btn-confirm:hover {
  background: var(--ba-blue-dark);
}

.ba-btn-confirm:disabled {
  background: #CBD5E1;
  color: #94A3B8;
  cursor: not-allowed;
}

/* List details Rates modal */
.ba-list-modal {
  width: 580px;
  max-width: 90vw;
  height: 420px;
...
.student-select-card.is-selected {
  border-color: var(--ba-blue);
  background: #E1F0FF;
  box-shadow: 0 0 0 3px rgba(18, 138, 250, 0.15);
}
...
.ba-modal-footer {
  padding: 12px 16px;
  background: #F1F7FC;
  border-top: 2px solid #C0D6E8;
  display: flex;
  gap: 12px;
  justify-content: flex-end;
}

.ba-modal-footer button {
  height: 38px;
  padding: 0 20px;
  border-radius: 6px;
}

/* CSS Backup Background animation styling */
.ba-css-bg {
  position: absolute;
  inset: 0;
  background: linear-gradient(135deg, #d3eafc 0%, #aed6f8 100%);
  overflow: hidden;
  z-index: 1;
}
...
.ba-bg-grids {
  ...
  inset: 0;
  background-image: 
    linear-gradient(90deg, rgba(18, 138, 250, 0.05) 1px, transparent 1px),
    linear-gradient(0deg, rgba(18, 138, 250, 0.05) 1px, transparent 1px);
...
.ba-shard-1 { width: 40px; height: 40px; left: 10%; top: 20%; animation-delay: 0s; }
.ba-shard-2 { width: 60px; height: 60px; right: 15%; top: 30%; animation-delay: 2s; }
.ba-shard-3 { width: 30px; height: 30px; left: 30%; bottom: 15%; animation-delay: 4s; }
.ba-shard-4 { width: 50px; height: 50px; right: 25%; bottom: 20%; animation-delay: 1s; }

@keyframes float-shard {
  0%, 100% { transform: translateY(0) rotate(45deg) scale(1); opacity: 0.25; }
  50% { transform: translateY(-20px) rotate(60deg) scale(1.1); opacity: 0.5; }
}
